...
const OlehOlehPage = () => {
  ...
  // Regional categories for the feature boxes
  const regions = [
    { name: 'Jawa', icon: '🏛️', desc: 'Batik, Gudeg, Bakpia', color: 'from-amber-500 to-orange-600' },
    { name: 'Bali', icon: '🌺', desc: 'Kerajinan, Kopi, Pie', color: 'from-rose-500 to-pink-600' },
    { name: 'Sumatera', icon: '🌿', desc: 'Rendang, Kopi, Songket', color: 'from-emerald-500 to-green-600' },
    { name: 'Kalimantan', icon: '🦚', desc: 'Mandau, Amplang, Manik', color: 'from-cyan-500 to-teal-600' },
  ];
        ...
        <div className="relative container mx-auto px-4 py-16 md:py-24">
          <div className="text-center max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full border border-white/20 mb-6">
              <GlobeAsiaAustraliaIcon className="w-4 h-4 text-amber-200" />
              <span className="text-sm font-medium text-amber-100">Cita Rasa Nusantara</span>
            </div>
            ...
            <p className="text-lg text-amber-100/80 mb-10 max-w-xl mx-auto leading-relaxed">
              Nikmati berbagai pilihan oleh-oleh khas dari berbagai daerah di Indonesia, langsung dari produsen terpercaya.
            </p>

            {/* Search Bar */}
            <div className="max-w-lg mx-auto">
              <div className="relative group">
                <MagnifyingGlassIcon className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-amber-600 transition-colors" />
                <input
                  type="text"
                  placeholder="Cari oleh-oleh favoritmu..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-14 pr-6 py-4 bg-white rounded-2xl shadow-2xl shadow-amber-900/20 border-0 focus:ring-2 focus:ring-amber-500 outline-none text-gray-800 placeholder-gray-400 transition-all"
                />
              </div>
            </div>
          </div>
      ...
      <section className="container mx-auto px-4 -mt-6 mb-12 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {regions.map((region, i) => (
            <div
              key={i}
              className="bg-white rounded-2xl p-5 shadow-sm hover:shadow-lg border border-gray-100 hover:border-amber-200 transition-all duration-300 cursor-pointer group hover:-translate-y-1"
            >
              <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${region.color} flex items-center justify-center mb-3 shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                <span className="text-2xl">{region.icon}</span>
              </div>
              <h3 className="font-bold text-gray-900 mb-1">{region.name}</h3>
              <p className="text-xs text-gray-500">{region.desc}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        {loading ? (
          ...
          <div className="text-center py-24">
            <div className="w-20 h-20 mx-auto bg-amber-50 rounded-full flex items-center justify-center mb-6">
              <ShoppingCartIcon className="w-10 h-10 text-amber-500" />
            </div>
            ...
            <button
              ...
              className="px-6 py-2.5 bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-xl font-medium shadow-lg shadow-amber-500/30"
            >
              Reset Filter
            </button>
          </div>
            ...
            {products.map((product) => {
              ...
              return (
                <div
                  ...
                  className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border border-gray-100 hover:border-amber-200 group cursor-pointer hover:-translate-y-1"
                >
                  {/* Image */}
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0">
                      ...
                      >
                        <ShoppingCartIcon className="w-4 h-4 text-gray-500 hover:text-amber-600" />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    <h3 className="font-bold text-gray-900 text-sm mb-1 line-clamp-1 group-hover:text-amber-600 transition-colors">
                      {product.name}
                    </h3>
